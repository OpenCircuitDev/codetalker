...
# 2026-05-16 -- the daemon now has two windows:
#   * SESSION_VISIBILITY_WINDOW_SEC = 30 days  → entry shows in list
#   * TRANSCRIPT_LIVE_WINDOW_SEC    = 5 min    → entry marked is_live
# Fixture timestamps must be inside the 30-day window but OUTSIDE the
# 5-min freshness window so tests asserting is_live=False stay valid.
NOW_ISH = time.time() - 3600.0  # 1 hour ago: visible but not "live"
...
async def test_merged_includes_catalog_only_sessions(app):
    ...
    state.catalog._entries["catalog-only"] = CatalogEntry(
        ...
        transcript_path=Path("/t/catalog-only.jsonl"), last_modified=NOW_ISH,
    )
...
async def test_merged_marks_live_sessions(app):
    ...
    state.catalog._entries["both"] = CatalogEntry(
        ...
        transcript_path=Path("/t/both.jsonl"), last_modified=NOW_ISH,
    )
...
async def test_merged_includes_has_persistent_settings_flag(app):
    ...
    state.catalog._entries["with-persist"] = CatalogEntry(
        ...
        transcript_path=Path("/t/with-persist.jsonl"), last_modified=NOW_ISH,
    )
